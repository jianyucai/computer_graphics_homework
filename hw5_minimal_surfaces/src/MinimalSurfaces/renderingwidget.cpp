...
void RenderingWidget::Global(){
    cout<<"Global"<<endl;
    minimal_surface* m = new minimal_surface(this->ptr_mesh_);
    m->compute_minimal_surface(GLOBAL);

    // refresh
    this->update();
}

void RenderingWidget::Local(){
    cout<<"Local"<<endl;
    minimal_surface* m = new minimal_surface(this->ptr_mesh_);
    m->compute_minimal_surface(LOCAL);
